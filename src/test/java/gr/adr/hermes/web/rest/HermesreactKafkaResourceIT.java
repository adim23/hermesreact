...
import gr.adr.hermes.config.KafkaProperties;
...
class HermesreactKafkaResourceIT {
    ...
    void setup() {
        ...
        HermesreactKafkaResource kafkaResource = new HermesreactKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/hermesreact-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/hermesreact-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
